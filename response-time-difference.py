...
class CsrfInputParser(HTMLParser):
   #定义一个继承自HTMLParser的类，用于扫描HTML标签
    ...
    def handle_starttag(self, tag, attrs):
        if tag.lower() != "input":
            return
        attributes = dict(attrs)  #attrs是列表，表值为二元元组如("type","hidden")，因此可以把表值转换成字典的键值对格式，把列表变成字典以使用.get()方法，但重复的属性-值对会被覆盖
        if attributes.get("name") == "csrf" and attributes.get("value"):  #.get()方法若没找到对应的键或键值为空，会返回None或空字符串，如果使用attributes[name]，当"name"属性不存在时会抛keyError
